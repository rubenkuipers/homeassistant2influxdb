...
def main():
    ...
    parser.add_argument('--dtype', '-dt',
                        dest='datatype', action='store', required=False,
                        help='Data type is either sum or mean'
                             'Sum is used by some gas usage sensors, while mean is used for electricity sensors')                         
    ...
    if (args.table == "states"):
        ...
        if (args.datatype == 'sum'):
            cursor.execute("select statistic_id,sum,state_attributes.shared_attrs,'state_changed',created from statistics_meta,statistics,state_attributes where metadata_id= statistics_meta.id and sum!=0 and state_attributes.attributes_id = (select states.attributes_id from states where states.entity_id = statistic_id limit 1 )")
        else:
